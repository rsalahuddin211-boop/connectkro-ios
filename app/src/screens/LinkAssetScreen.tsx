...
import { getAssetCategoryIconName, listAssetCategories } from '../services/assetCategories';
import { AssetAttachmentError, createAssetAndAttachQuickResponse } from '../services/assets';
import { styles } from '../styles/LinkAssetScreen.styles';
import type { AssetCategory } from '../types/assetCategory';
import { getErrorMessage } from '../utils/error';
import { colors } from '../theme/colors';
import { SHOP_QR_STICKER_SUBSCRIPTION_URL } from '../config/app';

interface LinkAssetScreenProps {
...
export default function LinkAssetScreen({ quickResponseUniqueId, onClose }: LinkAssetScreenProps) {
  const { token } = useAuth();
  ...
  const handleSubmit = async () => {
    if (!token) {
      void dialog.alert({ title: 'Auth Error', message: 'Please sign in to continue.', tone: 'error' });
      return;
    }
    ...
    setSubmitting(true);
    try {
      const response = await createAssetAndAttachQuickResponse({
        token,
        name: name.trim(),
        categoryId,
        notes: notes.trim(),
        quickResponseUniqueId,
        imageUri: selectedImageUri,
      });

      await dialog.alert({
        title: 'Asset Saved',
        message: response?.message || 'Your asset was created successfully.',
        tone: 'success',
      });
      onClose();
    } catch (error: unknown) {
      if (error instanceof AssetAttachmentError && error.reason === 'SUBSCRIPTION_REQUIRED') {
        void dialog.open({
          title: 'Subscription required',
          message: 'You need an active subscription before linking a QR to an asset.',
          tone: 'warning',
          actions: [
      ...
      void dialog.alert({
        title: 'Error',
        message: getErrorMessage(error, 'Failed to create asset'),
        tone: 'error',
      });
    } finally {
      setSubmitting(false);
    ...
    <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}>
      ...
      >
        <View style={styles.formSection}>
          ...
          <View style={styles.fieldWrap}>
            <TextInput
              ...
              placeholder="e.g. Honda Civic, MacBook Pro, Trek Bike"
        ...
        <View style={styles.formSection}>
          ...
          <TouchableOpacity
            style={[
              selectedImageUri ? styles.imagePreviewCard : styles.imageUploadCard,
              imageError ? styles.imageUploadCardError : null,
            ]}
            activeOpacity={0.86}
            onPress={pickImage}
          >
            {selectedImageUri ? (
              <>
                <Image source={{ uri: selectedImageUri }} style={styles.previewImageLarge} resizeMode="cover" />
                <View style={styles.changeImageRow}>
                  <Ionicons name="create-outline" size={14} color={colors.primaryBase} />
